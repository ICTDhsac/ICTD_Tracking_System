...
<?php
  ...
  $and = 'AND year(CompletedDate) = '.$year;
  $months = array();
  $lowella = array();
  $eric = array();
  $august = array();	
  ...
  for( $m = 1; $m <= 12; $m++ ) {
    $sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'Lowella Viray' $and";
	//echo $sql;  
    $lvquery = $conn->query($sql);
	array_push($lowella, $lvquery->num_rows);

    $sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'Eric Emilio Azanes' $and";
	//echo $sql;
    $eaquery = $conn->query($sql);
    array_push($eric, $eaquery->num_rows);
	
	$sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'Agustin Sopoco' $and";
	//echo $sql; 
    $asquery = $conn->query($sql);
    array_push($august, $asquery->num_rows);
  
	$sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'John Henry Canas' $and";
    $jcquery = $conn->query($sql);
    array_push($johnrey, $jcquery->num_rows);
  
	$sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'Nathaniel Paul Bunag' $and";
    $nbquery = $conn->query($sql);
    array_push($nathan, $nbquery->num_rows);
	
	$sql = "SELECT * FROM task WHERE month(StartDate) = $m AND month(CompletedDate)=1  AND BucketName='Completed' AND AssignedTo = 'Lawrence Miguel Segui' $and";
